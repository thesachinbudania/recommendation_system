...
def get_recommendations(user_preferences: UserPreferences, items: list[Item], top_n: int = 10) -> list[Item]:
    ...
    ranked_items = sorted(enumerate(similarity_scores), key=lambda x: x[1], reverse=True)

    # Exclude items the user has already interacted with
    ranked_items = [items[i] for i, score in ranked_items if items[i].id not in user_preferences.watch_history]
...
def combine_attributes(item: Item) -> str:
    """
    Combine the attributes of an item into a single string for vectorization
    """
    combined_attributes = []
